...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Future<void> _loadData() async {
    ...
    setState(() {
      ...
      selectHabits = prefs.getStringList('notificationHabit') ?? [];
      selectedTimes = prefs.getStringList('notificationsTimes') ?? [];
    });
  }

  Future<void> _saveNotificationSettings() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setBool('notificationEnabled', notificationsEnabled);
    await prefs.setStringList('notificationHabits', selectHabits);
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Wrap(
              ...
              children: allHabitsMap.entries.map((entry) {
                ...
                return FilterChip(
                  ...
                  selected: selectHabits.contains(habit),
            ...
            SizedBox(height: 20),
            Text(
              'Selected Times for Notification',
              style: TextStyle(fontWeight: FontWeight.bold),
            ),
            SizedBox(height: 10),
            Wrap(
              spacing: 8,
              runSpacing: 8,
              children: ['Morning', 'Evening', 'Nigth'].map((time) {
                return FilterChip(
                  label: Text(time),
